      ...
      try {
        ...
        if (data) {
          setArticles(data.map(article => ({
            id: article.id,
            title: article.title,
            section: article.categories?.name || 'Constitutional',
            status: article.status.charAt(0).toUpperCase() + article.status.slice(1).replace('-', ' '),
  ...
  useEffect(() => {
    const fetchPendingCount = async () => {
      try {
        ...
        console.error('Error fetching pending advisory count:', error);
      }
    };
    fetchPendingCount();
  }, []);
    ...
    return (
      <DashboardLayout navigationItems={navigationItems} title="Update Articles" role="Legal Expert">
        <div className="flex flex-col items-center justify-center py-20">
          <Loader2 className="w-12 h-12 text-[#138808] animate-spin mb-4" />
          <p className="text-[#0A1F44] font-medium">Loading articles...</p>
        </div>
      </DashboardLayout>
    );
  }
