      ...
      try {
        if (id) {
          ...
          if (article) {
            ...
            setArticlesData({
              title: article.categories?.name || 'Constitutional',
              description: article.categories?.description || 'Legal education module',
              articles: [article]
            });
          }
        } else if (category) {
          // Fetch articles by category name/slug
          ...
          if (categoryData) {
            ...
            if (categoryData.articles?.length > 0) {
              setSelectedArticle(categoryData.articles[0]);
            }
          }
        }
      } catch (error) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#F8F9FA]">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-[#0A1F44] font-medium">Loading constitutional content...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#F8F9FA] ashoka-pattern">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-12">
        {articlesData && articlesData.articles?.length > 0 ? (
            ...
            <div className="lg:col-span-3">
              {selectedArticle && (
                ...
                >
                  ...
                  {/* Important Cases */}
                  {selectedArticle.important_cases && selectedArticle.important_cases.length > 0 && (
                    <div className="mb-8">
                      ...
                      <div className="space-y-4">
                        {selectedArticle.important_cases.map((caseItem, index) => (
                  ...
                  <div className="mt-8 p-6 bg-gradient-to-br from-[#0A1F44]/5 to-[#1A3A6B]/5 rounded-xl">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h4 className="text-[#0A1F44] mb-2">Legal Disclaimer</h4>
                        <p className="text-sm text-[#64748B] leading-relaxed">
                          This article is provided for educational purposes only. For specific legal advice, please consult with a registered legal professional via our Advisory Portal.
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="text-center py-20 glass-white rounded-2xl border border-[#0A1F44]/10">
            <FileText className="w-16 h-16 text-[#64748B]/30 mx-auto mb-4" />
            <h3 className="text-xl text-[#0A1F44] mb-2">Content Not Found</h3>
            <p className="text-[#64748B] mb-8">The requested constitutional content could not be located in our database.</p>
            <Link to={isCitizenPortal ? "/citizen" : "/"} className="px-6 py-3 bg-[#0A1F44] text-white rounded-lg hover:bg-[#1A3A6B] transition-all">
              Return to Dashboard
            </Link>
          </div>
        )}
