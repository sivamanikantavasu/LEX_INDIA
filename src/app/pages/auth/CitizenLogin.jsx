...
import { Shield, Mail, Lock, Eye, EyeOff, ArrowLeft, Users } from 'lucide-react';
...
export default function CitizenLogin() {
  ...
  const handleGuestLogin = () => {
    // Set guest authentication state
    localStorage.setItem('isAuthenticated', 'true');
    localStorage.setItem('userRole', 'citizen');
    localStorage.setItem('userEmail', 'guest@lexindia.gov.in');
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] opacity-5">
          ...
          >
            <svg viewBox="0 0 200 200" className="w-full h-full">
              <circle cx="100" cy="100" r="90" fill="none" stroke="#1A3A6B" strokeWidth="2" />
              <circle cx="100" cy="100" r="15" fill="#1A3A6B" />
              {[...Array(24)].map((_, i) => {
                ...
                return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="#1A3A6B" strokeWidth="2" />;
              })}
            </svg>
          </motion.div>
        </div>
      ...
      >
        ...
        <div className="glass-white rounded-2xl p-8 md:p-10 shadow-2xl border border-[#1A3A6B]/20">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-[#64748B]">Access your learning dashboard</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center justify-between text-sm">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 rounded border-[#1A3A6B]/20 text-[#1A3A6B] focus:ring-[#1A3A6B]" />
                <span className="text-[#64748B]">Remember me</span>
              </label>
              <a href="#" className="text-[#1A3A6B] hover:text-[#0A1F44] transition-colors">
                Forgot password?
              </a>
            </div>
            ...
            <button
              type="submit"
              className="w-full py-3.5 bg-gradient-to-r from-[#1A3A6B] to-[#0A1F44] text-white rounded-lg hover:shadow-lg transition-all hover-lift"
            >
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-sm">
              <span className="px-4 bg-white text-[#64748B]">New citizen?</span>
            </div>
          </div>

          <Link 
            to="/auth/citizen/signup"
          ...
          <button
            ...
            className="w-full py-3.5 bg-gradient-to-r from-[#FF9933] to-[#FFB366] text-white rounded-lg hover:shadow-lg transition-all hover-lift"
          >
            Continue as Guest
          </button>
